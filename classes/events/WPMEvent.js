class WPMEvent extends Event {
    constructor() {
        super();
        this.name = "WPM Test";
    }
    activate(gameWorld) {
        this.prompt = random(prompts).toLowerCase();
        this.judging = false;
        this.textField = createInput();
        this.textField.position(width - (width / 4), height / 2);
        this.winner = false;
    }
    update(gameWorld) {
        push();
        if (!this.winner) {
            ...
            textSize(20);
            fill('white');
            text(this.prompt, 0, 10, width - (width / 4));
            this.progressBar = new ProgressBar(millis() - this.startTime, this.runtime - 2500, width / 4, 20, color(255), color(255, 0, 0));
            this.progressBar.render(createVector(width - (width / 4), height - this.progressBar.maxHeight - 5));
        }
        textSize(30);
        if (!this.judging) {
            let errors = 0;
        ...
        text(`wpm ${~~this.wpm}`, width - (width / 4), height / 4);
        pop();
...
let prompts = [`CUPERTINO, CALIFORNIA Today, the European Commission announced a decision claiming the App Store has been a barrier to competition in the digital music market. The decision was reached despite the Commission’s failure to uncover any credible evidence of consumer harm, and ignores the realities of a market that is thriving, competitive, and growing fast.
